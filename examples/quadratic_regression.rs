...
fn main() {
    let x_data = ((array1!(0..100) / 50.) - 1.).reshape(&[100, 1]);
    let y_data = x_data.powi(2) + Array::randn(&[100, 1], 0., 0.01);
    ...
    model
        .call(x)
        .get_array()
        .write_csv("data/quadratic_regression_pred.csv");
    y.get_array()
        .write_csv("data/quadratic_regression_target.csv");
}
